...
const playerSchema = new mongoose.Schema({
    ...
    playerId: { type: String, unique: true, sparse: true }, // 💡 المعرف الثابت PXXXX
...
playerSchema.methods.getRegistrationStatus = function() { return this.registrationStatus; };
playerSchema.methods.getCurrentLocation = function() { return this.currentLocation || 'forest'; };
...
playerSchema.methods.removeItem = function(id, quantity = 1) {
    ...
    if (itemIndex !== -1) {
        ...
        else { this.inventory.splice(itemIndex, 1); }  
        return true;
    }
    return false;
};
...
playerSchema.methods.isAlive = function() { return (this.health || 0) > 0; };
...
playerSchema.methods.getEquippedItemStats = function(itemsData) {
    const totalStats = {
        damage: 0, defense: 0, maxHealth: 0, maxMana: 0, maxStamina: 0, critChance: 0, healthRegen: 0,
    };
...
playerSchema.methods.getTotalStats = function(itemsData) {
    const baseStats = {
        damage: 10 + ((this.level || 1) - 1) * 2, defense: 5 + ((this.level || 1) - 1) * 1,
        maxHealth: 100 + ((this.level || 1) - 1) * 20, maxMana: 50 + ((this.level || 1) - 1) * 10,
        maxStamina: 100, critChance: 5, healthRegen: 1, 
    };
    const equippedStats = this.getEquippedItemStats(itemsData);
    return {
        damage: baseStats.damage + (equippedStats.damage || 0), defense: baseStats.defense + (equippedStats.defense || 0),
        maxHealth: baseStats.maxHealth + (equippedStats.maxHealth || 0), maxMana: baseStats.maxMana + (equippedStats.maxMana || 0),
...
playerSchema.methods.equipItem = function(itemId, itemType, itemsData) {
    if (this.getItemQuantity(itemId) === 0) { return { error: `❌ لا تملك العنصر ${itemsData[itemId]?.name || itemId} لتجهيزه.` }; }
    const slotMap = { 'weapon': 'weapon', 'armor': 'armor', 'accessory': 'accessory', 'tool': 'tool' };
    const slot = slotMap[itemType] || null;
    if (!slot) { return { error: `❌ النوع "${itemType}" لا يمكن تجهيزه في خانة معدات.` }; }                                                                 
    const oldItemId = this.equipment[slot];
    if (oldItemId === itemId) { return { error: `❌ العنصر ${itemsData[itemId]?.name || itemId} مجهز بالفعل في خانة ${slot}.` }; }
    if (oldItemId) { this.equipment[slot] = null; }
    this.equipment[slot] = itemId;
    this.recalculateMaxStats(this.getEquippedItemStats(itemsData));
    return { success: true, message: `✅ تم تجهيز ${itemsData[itemId]?.name || itemId} في خانة ${slot}.`, oldItemId: oldItemId };
};

playerSchema.methods.unequipItem = function(slot, itemsData) {
    const validSlots = ['weapon', 'armor', 'accessory', 'tool'];
    if (!validSlots.includes(slot)) { return { error: '❌ الخانة غير صالحة. استخدم: weapon, armor, accessory, tool.' }; }
    const unequippedItem = this.equipment[slot];
    if (!unequippedItem) { return { error: `❌ لا يوجد شيء مجهز في خانة ${slot}.` }; }
    this.equipment[slot] = null;
    this.recalculateMaxStats(this.getEquippedItemStats(itemsData));
    return { success: true, message: `✅ تم نزع ${itemsData[unequippedItem]?.name || unequippedItem} من خانة ${slot}.` };
};
...
playerSchema.methods.getGatherEfficiency = function() { return (this.skills && this.skills.gathering) || 1; };
playerSchema.methods.useMana = function(amount) { const currentMana = this.mana || 0; if (currentMana >= amount) { this.mana = currentMana - amount; return true; } return false; };
playerSchema.methods.restoreMana = function(amount) { this.mana = Math.min((this.mana || 0) + amount, this.maxMana || 50); };

// ========== دوال ثابتة (Static Methods) ==========

// 🆕 تحديث: العثور على آخر ID رقمي (P1001, P1002, ...)
playerSchema.statics.getLastNumericId = async function() {
    const lastPlayer = await this.findOne({ playerId: { $ne: null } }).sort({ createdAt: -1 }).exec();
    let lastNumericId = 1000;
    if (lastPlayer && lastPlayer.playerId) {
        const match = lastPlayer.playerId.match(/P(\d+)/);
        if (match && match[1]) {
            lastNumericId = parseInt(match[1], 10);
        }
    }
    // الزيادة والتأكد من أنها لا تقل عن 1001
    return Math.max(lastNumericId + 1, 1001);
};

// 🆕 دالة ثابتة للبحث عن لاعب باستخدام أي معرف (PID, UID, Name)
playerSchema.statics.findByIdentifier = async function(identifier) {
    if (!identifier) return null;
    const searchKey = identifier.toUpperCase();

    // البحث عن طريق: playerId (PXXXX) أو userId (Facebook ID) أو الاسم
...
// 🆕 تحديث: إنشاء لاعب جديد وتخصيص playerId متسلسل
playerSchema.statics.createNew = async function(userId, name) {
    try {
        const existingPlayer = await this.findOne({ userId });
        if (existingPlayer) {
             // 💡 إذا عاد اللاعب، استخدم نفس playerId القديم
             return existingPlayer;
        }

        const nextNumericId = await this.getLastNumericId();
        const newPlayerId = `P${nextNumericId}`; // مثال: P1001

        const player = new this({
            userId, name, registrationStatus: 'pending', gender: null, playerId: newPlayerId, 
            approvedAt: null, approvedBy: null,
            level: 1, experience: 0, gold: 50, health: 100, maxHealth: 100, mana: 50, maxMana: 50, stamina: 100, maxStamina: 100,
            lastStaminaAction: Date.now(), currentLocation: 'forest',
            inventory: [{ id: 'wood', name: 'خشب', type: 'resource', quantity: 5 }, { id: 'stone', name: 'حجر', type: 'resource', quantity: 3 }],
            skills: { gathering: 1, combat: 1, crafting: 1 }, equipment: { weapon: null, armor: null, accessory: null, tool: null },
        ...
        if (error.code === 11000) { const existingPlayer = await this.findOne({ userId }); if (existingPlayer) { return existingPlayer; } }  
        throw error;
    }
};

playerSchema.statics.findByUserId = async function(userId) { return await this.findOne({ userId }); };
playerSchema.statics.getTopPlayers = async function(limit = 10) { return await this.find({ banned: false, registrationStatus: 'completed' }).sort({ level: -1, experience: -1, gold: -1 }).limit(limit); };
playerSchema.statics.getPendingPlayers = async function() { return await this.find({ registrationStatus: 'pending' }).select('userId name createdAt'); };
playerSchema.virtual('requiredExp').get(function() { return (this.level || 1) * 100; });
playerSchema.virtual('expProgress').get(function() { const exp = this.experience || 0; const required = this.requiredExp; return Math.floor((exp / required) * 100) || 0; });
playerSchema.virtual('inventoryCount').get(function() { if (!this.inventory) return 0; return this.inventory.reduce((total, item) => total + item.quantity, 0); });
playerSchema.virtual('inventoryTypes').get(function() { if (!this.inventory) return 0; return this.inventory.length; });
